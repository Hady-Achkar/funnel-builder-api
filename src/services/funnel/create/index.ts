...
export const createFunnel = async (
  ...
  if (data.slug) {
    ...
    const isAutoGeneratedName = uniqueName === "New Site" || /^New Site \d+$/.test(uniqueName);

    if (isAutoGeneratedName) {
      // For auto-generated names, use sequential site slug
      const dateSlug = await generateDateSlug(workspace.id);
