...
export class AzureBlobStorageService {
  ...
  private loadConfig(): AzureBlobConfig {
    ...
    const containerName = process.env.AZURE_STORAGE_CONTAINER_NAME || 'template-images';
  ...
  async uploadBuffer(
    ...
    const fileExtension = this.getFileExtension(options.contentType || 'image/jpeg');
  ...
  async uploadFromUrl(
    ...
    try {
      ...
      const contentType = response.headers.get('content-type') || options.contentType || 'image/jpeg';
  ...
  generateFileName(originalName: string, templateId: number, imageType: string): string {
