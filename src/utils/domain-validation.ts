...
export function validateHostname(hostname: unknown): string {
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    if (!/^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/.test(label)) {
      throw new Error(
        `Invalid domain label: "${label}". Labels must start and end with letter/number and contain only letters, numbers or hyphens`
      );
    }
  }
  return trimmed;
...
export function validateSubdomain(sub: unknown): string {
  ...
  if (reservedNames.includes(trimmed)) {
    throw new Error(
      `"${trimmed}" is a reserved subdomain name and cannot be used`
    );
  }
  return trimmed;
}
