...
import { hasPermissionToCreateFunnel } from "../helpers";
import { 
  generateSlug, 
  generateDateSlug, 
  generateUniqueSlug 
...
export const createFunnel = async (
  ...
  try {
    ...
    if (validatedData.slug) {
      ...
      const isAutoGeneratedName = validatedData.name === format(new Date(), "dd.MM.yyyy HH:mm");
    ...
    if (error instanceof Error) {
      if (
        ...
        throw new Error(
          `A funnel with the name "${validatedData.name || 'provided'}" already exists in this workspace. Please choose a different name.`
        );
      }
