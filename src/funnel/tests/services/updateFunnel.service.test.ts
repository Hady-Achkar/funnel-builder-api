...
describe("FunnelService.updateFunnel", () => {
  ...
  it("should require user authentication", async () => {
    await expect(FunnelService.updateFunnel(testFunnel.id, 0, { name: "New Name" })).rejects.toThrow("Please provide userId.");
  });

  it("should validate funnelId", async () => {
    await expect(FunnelService.updateFunnel(-1, testUser.id, { name: "New Name" })).rejects.toThrow("Invalid input");
  });

  it("should throw error for non-existent funnel", async () => {
    await expect(FunnelService.updateFunnel(999999, testUser.id, { name: "New Name" })).rejects.toThrow("Funnel not found.");
  });

  it("should throw access denied for unauthorized user", async () => {
    const otherUser = await TestHelpers.createTestUser();
    await expect(FunnelService.updateFunnel(testFunnel.id, otherUser.id, { name: "New Name" })).rejects.toThrow("You can't update this funnel.");
  });

  it("should throw error when no data provided", async () => {
    await expect(FunnelService.updateFunnel(testFunnel.id, testUser.id, {})).rejects.toThrow("Nothing to update.");
  });

  it("should throw error when same data provided (no changes)", async () => {
    await expect(FunnelService.updateFunnel(testFunnel.id, testUser.id, { 
      name: "Original Funnel",
      status: $Enums.FunnelStatus.DRAFT 
  ...
  it("should update funnel name successfully", async () => {
    const result = await FunnelService.updateFunnel(testFunnel.id, testUser.id, {
