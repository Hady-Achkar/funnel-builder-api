...
describe("FunnelService.getFunnelById", () => {
  ...
  it("should require user authentication", async () => {
    ...
    await expect(FunnelService.getFunnelById(testFunnel.id, 0)).rejects.toThrow(
  ...
  it("should return null for non-existent funnel", async () => {
    await expect(
      FunnelService.getFunnelById(999999, testUser.id)
  ...
  it("should throw access denied error for unauthorized access (different user)", async () => {
    ...
    await expect(
      FunnelService.getFunnelById(testFunnel.id, otherUser.id)
    ).rejects.toThrow("Funnel not found.");
  });
  ...
  it("should return cached data when available", async () => {
    ...
    const firstResult = await FunnelService.getFunnelById(
      testFunnel.id,
      testUser.id
    );
    ...
    const secondResult = await FunnelService.getFunnelById(
      testFunnel.id,
      testUser.id
    );
