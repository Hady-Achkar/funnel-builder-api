...
export class DomainController {
  ...
  static async createCustomDomain(
    ...
    try {
      ...
      res.status(201).json({
        message:
          "Custom domain created. Please add ALL of the following DNS records at your domain provider.",
        domain,
        setupInstructions: {
          records: [domain.ownershipVerification, domain.dnsInstructions],
          nextSteps: [
  ...
  static async getVerificationInstructions(
    ...
    try {
      ...
      const instructions = await DomainService.getVerificationInstructions(
        domainId,
        userId
      );
      ...
      res
        .status(500)
        .json({ error: "Failed to get verification instructions" });
    }
  }
  ...
  static async getPublicFunnel(req: Request, res: Response): Promise<void> {
    try {
      ...
      const funnel = await DomainService.getPublicFunnel(
        hostname,
        parseInt(funnelId)
      );
