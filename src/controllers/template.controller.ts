...
export class TemplateController {
  ...
  async createTemplate(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const { name, description, categoryId, funnelId, tags, isPublic } = req.body;
  ...
  async createFunnelFromTemplate(req: AuthRequest, res: Response): Promise<void> {
    try {
      const userId = req.userId;
      if (!userId) {
        res.status(401).json({ error: 'Authentication required' });
  ...
  async getTemplatePreview(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const preview = await funnelFromTemplateService.getTemplatePreview(parseInt(id));
      res.json(preview);
    } catch (error) {
  ...
  async validateTemplate(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const validation = await funnelFromTemplateService.validateTemplate(parseInt(id));
      res.json(validation);
    } catch (error) {
  ...
  async duplicateFunnelAsTemplate(req: AuthRequest, res: Response): Promise<void> {
    try {
      const userId = req.userId;
      if (!userId) {
        res.status(401).json({ error: 'Authentication required' });
