...
import { mockPrisma, createMockPage, setupPageServiceTest } from "./test-setup";

describe("PageService.deletePage", () => {
  setupPageServiceTest();

  it("should validate input parameters with Zod", async () => {
    await expect(PageService.deletePage({ pageId: 0 }, 1)).rejects.toThrow(
      "Invalid input: Page ID must be positive"
    );

    await expect(PageService.deletePage({ pageId: 1 }, 0)).rejects.toThrow(
      "User ID is required"
    );
  });
  ...
  it("should throw error for non-existent page", async () => {
    ...
    await expect(PageService.deletePage({ pageId: 999 }, 1)).rejects.toThrow(
      "Page deletion failed: Page not found or you don't have access"
    );
  });
  ...
  it("should prevent deleting the last page in funnel", async () => {
    ...
    await expect(PageService.deletePage({ pageId: 1 }, 1)).rejects.toThrow(
      "Page deletion failed: You must have at least one page in the funnel"
    );
  });
