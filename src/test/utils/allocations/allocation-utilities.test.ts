...
describe('Allocation Utilities', () => {
  ...
  describe('UserWorkspaceAllocations', () => {
    describe('getBaseAllocation', () => {
      it('should return 1 workspace for FREE plan', () => {
        const allocation = UserWorkspaceAllocations.getBaseAllocation(UserPlan.FREE);
        expect(allocation).toBe(1);
      });

      it('should return 1 workspace for BUSINESS plan', () => {
        const allocation = UserWorkspaceAllocations.getBaseAllocation(UserPlan.BUSINESS);
        expect(allocation).toBe(1);
      });
  ...
  describe('WorkspaceFunnelAllocations', () => {
    describe('getBaseAllocation', () => {
      ...
      it('should return 1 funnel for BUSINESS workspace', () => {
        const allocation = WorkspaceFunnelAllocations.getBaseAllocation(UserPlan.BUSINESS);
        expect(allocation).toBe(1);
      });
  ...
  describe('FunnelPageAllocations', () => {
    describe('getBaseAllocation', () => {
      it('should return 35 pages for all plan types', () => {
        ...
        expect(FunnelPageAllocations.getBaseAllocation(UserPlan.BUSINESS)).toBe(35);
        expect(FunnelPageAllocations.getBaseAllocation(UserPlan.AGENCY)).toBe(35);
      });
    });
  ...
  describe('WorkspaceSubdomainAllocations', () => {
    describe('getBaseAllocation', () => {
      it('should return 1 subdomain for all plan types', () => {
        expect(WorkspaceSubdomainAllocations.getBaseAllocation(UserPlan.FREE)).toBe(1);
        expect(WorkspaceSubdomainAllocations.getBaseAllocation(UserPlan.BUSINESS)).toBe(1);
        expect(WorkspaceSubdomainAllocations.getBaseAllocation(UserPlan.AGENCY)).toBe(1);
      });
    });
  ...
  describe('Integration Tests', () => {
    ...
    it('should handle complete BUSINESS plan with add-ons scenario', () => {
      ...
      const customDomains = WorkspaceCustomDomainAllocations.getAllocationSummary(1, {
        workspacePlanType: businessPlan,
        addOns
      });
