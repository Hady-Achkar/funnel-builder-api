...
describe("Template System Integration Tests", () => {
  describe("Slug Utility Functions", () => {
    it("should create valid slugs from template names", () => {
      const testCases = [
        {
          input: "Lead Generation Template",
          expected: "lead-generation-template",
        },
  ...
  describe("Content Processing Logic", () => {
    it("should replace text placeholders correctly", () => {
      const content =
        "<div>Welcome to {{COMPANY_NAME}}</div><p>Contact {{COMPANY_NAME}} today!</p>";
      ...
      expect(processedContent).toBe(
        "<div>Welcome to My Business</div><p>Contact My Business today!</p>"
      );
    ...
    it("should handle multiple placeholder replacements", () => {
      const content =
        "Hello {{NAME}}, welcome to {{COMPANY}}. Visit {{WEBSITE}} for more info.";
      const replacements = {
        "{{NAME}}": "John",
        "{{COMPANY}}": "Acme Corp",
        "{{WEBSITE}}": "acme.com",
      ...
      expect(processedContent).toBe(
        "Hello John, welcome to Acme Corp. Visit acme.com for more info."
      );
    });

    it("should replace linking IDs in content", () => {
      const content =
        '<a href="old-link-1">Next</a><form action="old-link-2">Submit</form>';
      const linkingMap = {
      ...
      expect(processedContent).toBe(
        '<a href="new-link-abc">Next</a><form action="new-link-def">Submit</form>'
      );
  ...
  describe("Content Validation Logic", () => {
    it("should estimate content complexity correctly", () => {
      const testCases = [
        ...
        {
          content: "<div>".repeat(50) + "lots of content".repeat(20),
          expected: 4,
        },
      ];
      ...
      testCases.forEach(({ content, expected }) => {
        ...
        const result = content
          ? Math.max(1, Math.floor((htmlTags + textBlocks) / 20))
          : 1;
    ...
    it("should validate page ordering correctly", () => {
      ...
      testCases.forEach(({ pages, isSequential }) => {
        ...
        const result =
          JSON.stringify(pages) === JSON.stringify(expectedSequential);
  ...
  describe("Filename Generation Logic", () => {
    it("should generate properly formatted filenames", () => {
      ...
      const testCases = [
        ...
        {
          input: {
            originalName: "Special@Chars!.png",
            templateId: 456,
            imageType: "preview",
          },
    ...
    it("should generate unique filenames", () => {
      const generateBasicFileName = (name: string) =>
