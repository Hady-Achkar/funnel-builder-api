...
describe("CreatePaymentLinkController.createPaymentLink - Integration Tests", () => {
  ...
  describe("PLAN_PURCHASE - No Affiliate", () => {
  ...
  describe("PLAN_PURCHASE - With Affiliate", () => {
    it("should include affiliate data with commissionPercentage (NOT affiliateAmount)", async () => {
      ...
      expect(payload.custom_data.affiliateLink.affiliateAmount).toBeUndefined();

      // Verify JWT structure
      const decoded = jwt.verify(testAffiliateToken, process.env.JWT_SECRET!) as any;
      expect(decoded.commissionPercentage).toBe(15);
      expect(decoded.affiliateAmount).toBeUndefined(); // Should not exist
    });
  });

  describe("WORKSPACE_PURCHASE - Success Cases", () => {
  ...
  describe("Validation Errors", () => {
    it("should reject if buyer email already exists in database", async () => {
      ...
      expect(next).toHaveBeenCalled();
      const error = next.mock.calls[0][0];
      expect(error.message).toMatch(/email.*already exists/i);
