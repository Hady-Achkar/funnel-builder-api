...
describe("updateTheme Service", () => {
  ...
  it("should validate themeId parameter", async () => {
    await expect(
      updateTheme({ themeId: 0 }, 1, { name: "Test" })
    ).rejects.toThrow("Theme ID must be positive");
  });
  ...
  it("should validate theme exists", async () => {
    ...
    await expect(
      updateTheme({ themeId: 999 }, 1, { name: "Test" })
  ...
  it("should validate theme has funnel association", async () => {
    ...
    await expect(
      updateTheme({ themeId: 1 }, 1, { name: "Test" })
    ).rejects.toThrow("You don't have permission to update this theme");
  });
  ...
  it("should successfully update theme", async () => {
    ...
    const result = await updateTheme({ themeId: 1 }, 1, {
      name: "New Theme",
